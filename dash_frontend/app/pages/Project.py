    ...
    card = dbc.Card(
        dbc.CardBody(
            [
                ...
                html.P(d["description"], className="card-text",),
                html.P(className="card-text",),
                dbc.CardLink(
                    "Go to Dataset", href=f"http://localhost/dash/datasets/{d['id']}"
                ),
            ]
        ),
        # style={"width": "18rem"},
    ...
    card = dbc.Card(
        dbc.CardBody(
            [
                ...
                html.H4("Add dataset"),
                dcc.Input(id="new-dataset-name", type="text", value="New dataset name"),
                dcc.Input(
                    id="new-dataset-location", type="text", value="New dataset location"
                ),
                dbc.Button("Add new dataset", id="add-new-dataset",),
                html.H3(
                    dbc.CardLink(
                        "Datasets",
                        href=f"http://localhost/dash/projects/{p['id']}/datasets",
                    ),
                ),
                dbc.Row([get_comp_for_dataset(d) for d in p["datasets"]]),
            ]
        ),
        # style={"width": "18rem"},
    )
...
def add_new_dataset(n_clicks, pathname, new_dataset_name, new_dataset_location):
    if n_clicks:
        ...
        dataset_in = {"title": new_dataset_name, "location": new_dataset_location}
