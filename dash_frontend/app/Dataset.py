...
class Dataset:
    ...
    def get_label(self, segment_id, slice_id):
        try:
            ...
            labels_folder = f"{ROOT_DATA_FOLDER}{self.segmentations[segment_id]['location']}"
        except:
            logging.debug(f"No segment_id key {segment_id} in {self.segmentations.keys()}")
            labels_folder = None
